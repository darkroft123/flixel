...
class FlxSound extends FlxBasic {
	...
	/**
	 * Whether this sound was paused by the SoundFrontEnd handler (focus lost).
	 * Used by SoundFrontEnd.pause() / resume() to track which sounds it paused.
	 * @since raltyMod
	 */
	@:allow(flixel.system.frontEnds.SoundFrontEnd)
	var _pausedByHandler:Bool = false;

	/**
	 * Whether this sound was playing before SoundFrontEnd paused it.
	 * Used so resume() only resumes sounds that were actually playing.
	 * @since raltyMod
	 */
	@:allow(flixel.system.frontEnds.SoundFrontEnd)
	var _pausedPlay:Bool = false;

	/**
	 * Reference to the FlxSoundData used by this sound (for the SoundFrontEnd cache system).
	 * May be null if the sound was loaded without going through FlxSoundData.
	 * @since raltyMod
	 */
	@:allow(flixel.system.frontEnds.SoundFrontEnd)
	public var data:FlxSoundData = null;

	/**
	 * Public read-only access to the internal AudioSource.
	 * Required by SoundFrontEnd to directly pause/resume the native audio source.
	 * @since raltyMod
	 */
	public var source(get, never):AudioSource;
	inline function get_source():AudioSource return _source;
	...
	public function reset():Void {
		...
		// Reset SoundFrontEnd handler state
	...
	/**
	 * Alias for `loadStream()` — loads a sound from a URL.
	 * Required by `SoundFrontEnd.load()` which calls this method by name.
	 *
	 * @param   soundURL     A string representing the URL of the sound file.
	 * @param   looped       Whether or not this sound should loop endlessly.
	 * @param   autoDestroy  Whether to destroy this FlxSound when it finishes.
	 * @param   onComplete   Called when the sound finished playing.
	 * @param   onLoad       Called when the sound finished loading.
	 * @return  This FlxSound instance.
	 */
	public inline function loadFromURL(soundURL:String, looped = false, autoDestroy = false, ?onComplete:Void->Void, ?onLoad:Void->Void):FlxSound {
		return loadStream(soundURL, looped, autoDestroy, onComplete, onLoad);
	}
	...
	#if FLX_SOUND_SYSTEM
	/**
	 * Called by SoundFrontEnd when the game regains focus.
	 * Only resumes if this sound was paused by the handler (not by game logic).
	 */
	@:allow(flixel.system.frontEnds.SoundFrontEnd)
	function onFocus():Void {
		// Legacy path — SoundFrontEnd now handles this directly via _pausedByHandler.
		// Kept for compatibility with any external code that calls it.
		if (_pausedByHandler) {
			_pausedByHandler = false;
			if (_pausedPlay) {
				_pausedPlay = false;
	...
	/**
	 * Called by SoundFrontEnd when the game loses focus.
	 * Pauses only if the sound was actually playing.
	 */
	@:allow(flixel.system.frontEnds.SoundFrontEnd)
	function onFocusLost():Void {
		...
		if (!_pausedByHandler) {
			_pausedPlay = playing;
			_pausedByHandler = true;
			if (_pausedPlay) pause();
		}
	}

	/**
	 * Reapplies the current pitch based on timeScale.
	 * Called by SoundFrontEnd when FlxG.timeScale changes.
	 * @since raltyMod
	 */
	@:allow(flixel.system.frontEnds.SoundFrontEnd)
	...
	/**
	 * Reapplies the current volume transform.
	 * Called by SoundFrontEnd when global volume changes.
	 * @since raltyMod
	 */
	@:allow(flixel.system.frontEnds.SoundFrontEnd)
